 ...
 *   do_change_pid: change the pid to a process
 */
 ...
 *                           do_change_pid                                  *
 *==========================================================================*/
PUBLIC pid_t do_change_pid(void)
{
  struct mproc *rmc;
  if (m_in.pid == 1)  /*Si es el init, return error*/
    return -1;
  /*Buscar si algun proceso tiene el pid que queremos poner*/
  for (rmc = &mproc[0]; rmc < &mproc[NR_PROCS]; rmc++)
  {
     if(rmc->mp_pid == m_in.m1_i2)
        return -1;
  }

  /*Buscamos si existe algun proceso con el pid a cambiar*/
  for (rmc = &mproc[0]; rmc < &mproc[NR_PROCS]; rmc++)
  {
     if(rmc->mp_pid == m_in.pid)
     {
	if (rmc->mp_flags & PRIV_PROC)
	{
	   return -1;
	}
         rmc->mp_pid = m_in.m1_i2;
         return rmc->mp_pid;
     }
  }
  return -1;
